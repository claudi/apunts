...
\section{Sèries numèriques}
    ...
    \begin{proof}
        ...
        Per la definició de \myref{def:sèrie convergent} això és que
        \[
    ...
    \begin{solution}
        ...
        \]
        Per tant tenim que la successió~\((S_{N})\) està fitada, i degut a que~\(S_{N+1}=S_{N}+\frac{1}{(N+1)^{\alpha}}\) tenim que és creixen i pel \myref{thm:Weierstrass màxims i mínims múltiples variables} tenim que és convergent.
        Per tant per la definició de \myref{def:sèrie convergent} tenim que la sèrie \eqref{ex:sèrie harmònica:eq1} és convergent.
    ...
    \begin{proof}
        ...
        Per la definició de \myref{def:límit} tenim que existeix un natural~\(n_{0}\) tal que per a tot~\(n>n_{0}\) es satisfà
        \[
        ...
        Per la definició de \myref{def:límit} tenim que per a tot~\(\varepsilon>0\) existeix un natural~\(n_{0}\) tal que
        \[
    ...
    \begin{proposition}[Criteri de l'arrel]
        ...
        tal que
        \begin{enumerate}
            \item\label{prop:criteri de l'arrel:enum1}~\(L<1\).
            Aleshores la sèrie~\(\sum_{n=0}^{\infty}a_{n}\) és convergent.
            \item\label{prop:criteri de l'arrel:enum2}~\(L>1\).
            Aleshores la sèrie~\(\sum_{n=0}^{\infty}a_{n}\) és divergent.
        \end{enumerate}
    \end{proposition}
    \begin{proof}
        Comencem veient el punt \eqref{prop:criteri de l'arrel:enum1}.
        ...
        Veiem ara el punt \eqref{prop:criteri de l'arrel:enum2}.
        Suposem doncs que~\(L>1\).
    ...
    \begin{solution}
        ...
        Per tant per la definició de \myref{def:sèrie de termes positius} tenim que~\(\sum_{n=1}^{\infty}a_{n}\) és una sèrie de termes positius.
    ...
    \begin{proof}
        ...
        Suposem doncs que~\(L<1\).
        Prenem un~\(\varepsilon>0\) tal que~\(L+\varepsilon<1\).
        Aleshores per la definició de \myref{def:límit} tenim que existeix un~\(n_{0}\) tal que per a tot~\(n>n_{0}\) es satisfà
        \[
        ...
        Suposem doncs que~\(L>1\).
        Prenem un~\(\varepsilon>0\) tal que~\(L-\varepsilon>1\).
        Aleshores per la definició de \myref{def:límit} tenim que existeix un~\(n_{0}\) natural tal que per a tot~\(n>n_{0}\) es satisfà
        \[
            \abs{\frac{a_{n+1}}{a_{n}}-L}<\varepsilon,
        \]
        i per tant per a tot~\(n>n_{0}\) tenim
...
%            \[\alpha\log\left(\frac{x}{x+1}\right)=\log\left(x^{\alpha}\right)-\alpha\log(x+1)\]
    ...
    \begin{proof}
        ...
        Suposem doncs que~\(L>1\).
        Prenem un~\(\varepsilon>0\) tal que~\(L-\varepsilon>1\).
        Aleshores per la definició de \myref{def:límit} tenim que existeix un natural~\(n_{0}\) tal que per a tot~\(n>n_{0}\) es satisfà
        \[
    ...
    \begin{solution}
        ...
        \end{align*}
        Per tant, pel \myref{prop:criteri logarítmic} trobem que si~\(\alpha>\e\) la sèrie \eqref{ex:sèrie de la inversa de alpha elevat a un logarítme:eq1} és convergent, i si~\(\alpha<\e\) la sèrie \eqref{ex:sèrie de la inversa de alpha elevat a un logarítme:eq1} és convergent.
        Estudiem el cas~\(\alpha=\e\).
        Això és la sèrie
        \[
    ...
    \begin{proof}
        ...
        Com que, per hipòtesi, la sèrie és de termes positius, per la definició de \myref{def:sèrie convergent} tenim que existeix un real~\(C\) tal que per a tot~\(N\) natural es satisfà
        \[
    ...
    \begin{proof}
        ...
        També tenim per \eqref{thm:criteri de Leibniz per sèries alternades:eq2} que tenen el mateix límit, i per tant la successió~\((S_{N})\) és convergent, i per la definició de \myref{def:sèrie convergent} trobem que la sèrie~\(\sum_{n=0}^{\infty}(-1)^{n}a_{n}\) és convergent, com volíem veure.
        %REF
    \end{proof}
    ...
    \begin{solution}
        ...
        Per la definició de \myref{def:convergència absoluta d'una sèrie} això és estudiar la convergència de la sèrie
        \begin{equation}
    ...
    \begin{proof}
        ...
        és convergent, com volíem veure.
    \end{proof}
    \begin{example}%Buscar un altre exemple. Conflicte amb \myref{ex:sèrie absolutament convergent} %Crec que al final no, però comprovar amb calma (Claudi del futur)
    ...
    \begin{solution}
        ...
        \]
        és convergent.
        Per tant pel \myref{thm:Criteri d'Abel per sèries numèriques} tenim que la sèrie \eqref{ex:criteri d'Abel per sèries numèriques:eq1} és convergent.
    \end{solution}
    ...
    \begin{proof}
        ...
        \]
        i com que, per la definició de \myref{def:sèrie de termes positius}, tenim que~\(a_{n}\geq0\) per a tot~\(n\) natural trobem que
        \[
    ...
    \begin{proof}
        ...
        Per la definició de \myref{def:convergència absoluta d'una sèrie} tenim que la sèrie~\(\sum_{n=0}^{\infty}a_{n}\) és absolutament convergent si i només si les sèries~\(\sum_{n=0}^{\infty}a^{+}_{n}\) i~\(\sum_{n=0}^{\infty}a^{-}_{n}\) són convergents.
        Per tant trobem que
        \begin{align*}
    ...
    \begin{proof}
        ...
        Tenim per hipòtesi que la sèrie~\(\sum_{n=0}^{\infty}a_{n}\) una sèrie convergent però no absolutament convergent, i pel lema \myref{lema:Teorema de la reordenació de sèries de Riemann} trobem que les sèries~\(\sum_{n=0}^{\infty}a^{+}_{n}\) i~\(\sum_{n=0}^{\infty}a^{-}_{n}\) són divergents.
...
\section{Successions de funcions}
    ...
    \end{example}
    \begin{proof}
        Per l'exercici \myref{ex:convergència puntual d'una successió de funcions} tenim que la successió de funcions~\((f_{n})\) convergeix puntualment a la funció~\(f(x)=x\) en l'interval~\([1,2]\).
        Considerem el límit
        \begin{align*}
...
\section{Sèries de funcions}
    ...
    \begin{definition}[Sèries de potències]
        \labelname{sèrie de potències}\label{def:sèrie de potències}
