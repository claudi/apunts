...
\section{Espai de funcions contínues i acotades}
    ...
    \begin{notation}[Espai de funcions contínues i acotades]
        \labelname{espai de funcions contínues i acotades}\label{notation:espai-de-funcions-continues-i-acotades}
    ...
    \begin{definition}[Norma d'una funció]
        \labelname{norma d'una funció}\label{def:norma-duna-funcio}
...
%    \end{proposition}
    \begin{definition}[Successió de Cauchy]
        \labelname{successió de Cauchy}\label{def:successio-de-Cauchy-amb-normes}
    ...
    \begin{definition}[Espai mètric complet]
        \labelname{espai mètric complet}\label{def:espai-metric-complet}
    ...
    \end{definition}
    \begin{proposition}
        \label{prop:distancia-entre-funcions-acotades}
        ...
        \begin{enumerate}
            \item\label{prop:distancia-entre-funcions-acotades:enum1}~\(\distancia(f,g)\geq0\).
            \item\label{prop:distancia-entre-funcions-acotades:enum2}~\(\distancia(f,g)=0\) si i només si~\(f=g\).
            \item\label{prop:distancia-entre-funcions-acotades:enum3}~\(\distancia(f,g)\leq\distancia(f,h)+\distancia(h,g)\).
            \item\label{prop:distancia-entre-funcions-acotades:enum4}~\(\distancia(f,g)=\distancia(g,f)\).
        \end{enumerate}
    \end{proposition}
    \begin{proof}
        Comencem veient el punt \eqref{prop:distancia-entre-funcions-acotades:enum1}.
        Per la definició de \myref{def:norma-duna-funcio} tenim que
        \[
        ...
        Per la definició de \myref{def:norma-duna-funcio} ha de ser
        \[
            \sup_{x\in X}\norm{f(x)-g(x)}=0,
        \]
    ...
    \begin{definition}[Distància entre funcions]
        \labelname{distància entre funcions}\label{def:distancia-entre-funcions-acotades}
        ...
        Aquesta definició té sentit per la proposició \myref{prop:distancia-entre-funcions-acotades}.
    \end{definition}
    \begin{lemma}
        \label{thm:lespai-de-funcions-continues-i-acotades-es-un-espai-metric}
        Siguin~\(X\) amb la topologia~\(\tau\) un espai topològic i~\(T\) un compacte de~\(\mathbb{R}^{n}\).
    ...
    \begin{notation}
        ...
        Observem que això té sentit pel lema \myref{thm:lespai-de-funcions-continues-i-acotades-es-un-espai-metric}.
    \end{notation}
    \begin{theorem}
        \label{thm:lespai-de-funcions-continues-i-acotades-es-complet}
        Siguin~\(X\) amb la topologia~\(\tau\) un espai topològic i~\(T\) un compacte de~\(\mathbb{R}^{n}\).
    ...
    \begin{proof}
        ...
        Per la definició de \myref{def:successio-de-Cauchy-amb-normes} tenim que per a tots naturals~\(n\) i~\(m\) existeix un real~\(\varepsilon>0\) tal que
        \[
            \norm{f_{n}-f_{m}}<\varepsilon.
        \]
        Per la definició de \myref{def:distancia-entre-funcions-acotades} trobem que
        \[
        ...
        \[
            \norm{f_{n}(x)-f_{m}(x)}<\varepsilon
        \]
        i per la definició de \myref{def:successio-de-Cauchy-amb-normes} trobem que la successió~\((f_{n}(x))_{n\in\mathbb{N}}\) és de Cauchy, i per la definició de \myref{def:limit} trobem que la successió~\((f_{n}(x))_{n\in\mathbb{N}}\) és convergent.
        ...
        Per la definició de \myref{def:limit} tenim que existeix un real~\(\varepsilon>0\) tal que per a tot~\(n\) i~\(m\) enters amb~\(n,m\geq n_{0}\)
        \[
        ...
        \]
        i per la definició de \myref{def:convergencia-uniforme} tenim que~\((f_{n})_{n\in\mathbb{N}}\) convergeix uniformement a~\(f(x)\), i per la definició de \myref{def:espai-metric-complet} tenim que~\(\mathcal{C}_{b}(X,T)\) és complet.
    \end{proof}
    \subsection{Aplicacions contractives i punts fixes}
    \begin{definition}[Aplicació contractiva]
        \labelname{aplicació contractiva}\label{def:aplicacio-contractiva}
    ...
    \end{definition}
    \begin{observation}
        \label{obs:les-aplicacions-contractives-son-uniformement-continues}
    ...
    \begin{definition}[Punt fix]
        \labelname{punt fix d'una aplicació contractiva}\label{def:punt-fix-dun-aplicacio-contractiva}
    ...
    \end{definition}
    \begin{theorem}
        \label{thm:les-aplicacions-contractives-tenen-un-unic-punt-fix}
    ...
    \end{theorem}
    \begin{proof}
        Per la definició d'\myref{def:aplicacio-contractiva} tenim que existeix un real~\(0<k<1\) tal que per a tots~\(x\) i~\(y\) de~\(X\) es satisfà
        \begin{equation}
        ...
        \]
        i per la definició de \myref{def:successio-de-Cauchy-amb-normes} tenim que la successió~\((T^{n}(x))\) és una successió de Cauchy, i com que, per hipòtesi,~\(X\) és complet, tenim que existeix un element~\(p\) de~\(X\) tal que
        \begin{equation}
        ...
        \]
        Com que, per l'observació \myref{obs:les-aplicacions-contractives-son-uniformement-continues},~\(T\) és contínua, tenim que %REF
        \[
        ...
        \[
            T(p)=p,
        \]
        i per la definició de \myref{def:punt-fix-dun-aplicacio-contractiva} hem acabat.
    \end{proof}
    \begin{corollary}
    ...
    \begin{proof}
        ...
        Aleshores per la definició de \myref{def:punt-fix-dun-aplicacio-contractiva} tenim que
        \[
            T^{k}(x)=x
        \]
        i
        \[
            T^{k}(y)=y.
        \]
        Ara bé, pel Teorema \myref{thm:les-aplicacions-contractives-tenen-un-unic-punt-fix} tenim que ha de ser~\(x=y\), i per tant aquest punt és únic.
        % CONTINUAR

        Veiem ara que existeix un punt~\(x\) tal que~\(T(x)=x\).
        Tenim, per hipòtesi, que existeix un~\(k\) natural tal que~\(T^{k}\) és contractiva, i pel Teorema \myref{thm:les-aplicacions-contractives-tenen-un-unic-punt-fix} tenim que existeix un punt~\(x\) de~\(X\) tal que~\(T^{k}(x)=x\).
        Tenim també que
        \begin{equation}
...
\section{Teoremes d'existència i unicitat}
    ...
    \begin{definition}[Equació diferencial ordinària]
        \labelname{equació diferencial ordinària}\label{def:equacio-diferencial-ordinaria}
    ...
    \begin{definition}[Solució d'una equació diferencial ordinària]
        \labelname{solució d'un equació diferencial ordinària}\label{def:solucio-duna-equacio-diferencial-ordinaria}
        Siguin
        \[
            \dot{u}(t)=f(t,u(t))
        \]
    ...
    \end{definition}
    \begin{example}
        \label{ex:exemple-duna-equacio-diferencial-1}
    ...
    \begin{definition}[Problema de Cauchy]
        ...
        \labelname{solució d'un problema de Cauchy}\label{def:solucio-dun-problema-de-Cauchy}
        Sigui
        \[
            \dot{u}(t)=f(t,u(t))
        \]
        ...
        Això té sentit per la definició de \myref{def:solucio-duna-equacio-diferencial-ordinaria}.
    \end{definition}
    \begin{observation}
        \label{obs:solucio-general-dun-problema-de-Cauchy}
        Sigui
        \[\begin{cases*}
            \displaystyle \dot{u}(t)=f(t,u(t)) \\
            \displaystyle u(t_{0})=x_{0}
...
%                \displaystyle u(t_{0})=x_{0}.
%            \end{cases*}\]
%            Per la definició de \myref{def:solucio-dun-problema-de-Cauchy} trobem que\(u\) és solució de l'equació diferencial
%            \[\dot{u}(t)=f(t,u(t)),\]
%            i per la definició de \myref{def:solucio-duna-equacio-diferencial-ordinaria} trobem que existeix un interval~\(I\) tal que es satisfà~\(u'(t)=f(t,u(t))\) per a tot~\(t\in I\). Pel \myref{thm:Teorema-Fonamental-del-Calcul} trobem que
...
%        \end{proof}
    ...
    \begin{definition}[Funció Lipschitz respecte la segona variable]
        \labelname{funció Lipschitz respecte la segona variable}\label{def:funcio-Lipschitz-respecte-la-segona-variable}
    ...
    \end{definition}
    \begin{example}
        \label{ex:els-polinomis-son-funcions-Lipschitz-respecte-la-segona-variable}
    ...
    \begin{proof}
        ...
        \end{align*}
        Per l'observació \myref{obs:solucio-general-dun-problema-de-Cauchy} trobem que una funció~\(u(t)\) és solució del problema de Cauchy de l'enunciat si i només si %REVISAR
        \[
            T(u(t))=u(t).
        \]
        ...
        Per hipòtesi tenim que~\(f\) és Lipschitz respecte la segona variable, i per la definició de \myref{def:funcio-Lipschitz-respecte-la-segona-variable} trobem que existeix un real~\(K\geq0\) tal que
        \begin{equation}
        ...
        Tenim pel Teorema \myref{thm:la-norma-duna-integral-es-menys-que-lintegral-de-la-norma} que
        \begin{align*}
        ...
        \end{align*}
        Per tant, pel \myref{thm:principi-dinduccio} tenim que per a tot~\(m\) natural,~\(u\) i~\(v\) funcions de~\(X\) i~\(t\) de~\((t_{0}-a,t_{0}+a)\) es satisfà
        \[
            \norm{T^{m}(u(t))-T^{m}(v(t))}\leq\frac{K^{m}}{m!}\abs{t-t_{0}}^{m}\distancia(u,v),
        \]
        com volíem veure.
        ...
        \]%REF
        i per la definició de \myref{def:limit} tenim que existeix un natural~\(n_{0}\) tal que per a tot~\(n>n_{0}\) es satisfà
        \[
            0<{\frac{K^{m}}{m!}\abs{t-t_{0}}^{m}}<1.
        \]

        Per tant, per la definició d'\myref{def:aplicacio-contractiva} tenim que per a tot~\(n>n_{0}\) l'aplicació~\(T^{n}\) és una aplicació contractiva, i pel \corollari{} \myref{cor:si-una-funcio-te-una-potencia-contractiva-aleshores-aquesta-te-un-unic-punt-fix} tenim que existeix una única funció~\(u\) de~\(X\) tal que
        \[
            T(u(t))=u(t).\qedhere
        \]
    \end{proof}
    ...
    \begin{definition}[Família de funcions puntualment acotada]
        \labelname{família de funcions puntualment acotada}\label{def:familia-de-funcions-puntualment-acotada}
    ...
    \begin{definition}[Família de funcions equicontínua]
        \labelname{família de funcions equicontínua en un punt}\label{def:familia-de-funcions-equicontinua-en-un-punt}
        \labelname{família de funcions equicontínua}\label{def:familia-de-funcions-equicontinua}
        \labelname{família de funcions uniformement equicontínua}\label{def:familia-de-funcions-uniformement-equicontinua}
    ...
    \begin{lemma}[Teorema d'Arzelà-Ascoli]
        \labelname{Teorema d'Arzelà-Ascoli}\label{thm:Teorema-dArzela-Ascoli}
    ...
    \begin{proof}
        ...
        Per la definició de \myref{def:familia-de-funcions-equicontinua} tenim que per a tot punt~\(x\) de~\(X\) existeix un entorn~\(N_{x}\) de~\(x\) tal que per a tot~\(y\) de~\(N_{x}\) tenim, per a tot~\(n\) natural, que
        \[
        ...
        Com que, per hipòtesi, la~\((f_{n})_{n\in\mathbb{N}}\) és una successió de funcions puntualment acotada, per la definició de \myref{def:familia-de-funcions-puntualment-acotada} tenim que existeix un real~\(M\) tal que per a tot~\(n\) natural tenim
        \[
            \norm{f_{n}(x_{i})}<M,
        \]
        ...
        Per tant, per la definició de \myref{def:limit} trobem que existeix un element de~\(X\),~\(f^{k}(x_{i})\) tal que existeix un natural~\(n'_{\varepsilon}\) satisfent que per a tot~\(n>n'_{\varepsilon}\) es compleix
        \begin{equation}
        ...
        \]
        i per tant, per la definició de \myref{def:successio-de-Cauchy-amb-normes} trobem que la successió~\((f^{i+1}_{n_{i+1}+n})_{n\in\mathbb{N}}\) és de Cauchy per a tot~\(i\) natural, i com que pel Teorema \myref{thm:lespai-de-funcions-continues-i-acotades-es-complet} tenim que~\(\mathcal{C}_{b}(X,T)\) és un espai complet, i per la \myref{thm:condicio-de-Cauchy-per-successions-de-funcions} trobem que la parcial~\((f^{i+1}_{n_{i+1}+n})_{n\in\mathbb{N}}\) és uniformement convergent.
    \end{proof}
    ...
    \begin{proof} % https://arxiv.org/pdf/1202.1152.pdf (no)
        ...
        \]
        Aleshores pel \myref{thm:Teorema-daproximacio-polinomica-de-Weierstrass} trobem que existeixen~\(n\) successions de polinomis~\(\{(p_{m}^{i})_{m\in\mathbb{N}}\}_{i=1}^{n}\) tals que~\((p_{m}^{i})\) convergeix uniformement a~\(f_{i}\) per a tot~\(i\in\{1,\dots,n\}\).
        Per la proposició \myref{prop:una-successio-de-funcions-convergeix-uniformement-a-una-funcio-a-la-que-convergeixi-puntualment} trobem que~\((p_{m}^{i})\) convergeix puntualment a~\(f_{i}\) per a tot~\(i\in\{1,\dots,n\}\), i per la definició de \myref{def:convergencia-puntual} trobem que per a tot~\((t,x)\in\Omega\) tenim
        \[
        ...
        \end{equation}
        Per la definició de \myref{def:limit} trobem que per a tot~\(\varepsilon>0\) existeix un~\(n_{0}\) tal que per a tot~\(m>n_{0}\) es satisfà
        \[
        ...
        \[
            \norm{\varphi_{m}(t)}\leq\norm{x_{0}}+b.
        \]
        Aleshores per la definició de \myref{def:familia-de-funcions-puntualment-acotada} trobem que la successió de funcions~\((\varphi_{m+n_{0}})_{m\in\mathbb{N}}\) és una família de funcions puntualment acotada.

        Fixem un~\(\varepsilon>0\) i prenem dos~\(t\) i~\(t'\) de~\(I\) amb~\(t'<t\) tals que~\(\abs{t,t'}<\frac{\varepsilon}{M}\).
        Aleshores, per l'observació \myref{obs:solucio-general-dun-problema-de-Cauchy}, tenim que per a tot~\(m>n_{0}\)
        \begin{align*}
...
%                &=\norm{\smash{\int_{t_{0}}^{t'}}p_{m}(s,\varphi_{m}(s))\diff s+\int_{t'}^{t}p_{m}(s,\varphi_{m}(s))\diff s-\int_{t_{0}}^{t'}p_{m}(s,\varphi_{m}(s))\diff s} \tag{\ref{thm:podem-partir-les-integrals-Riemann}} \\
        ...
        \end{align*}
        i per la definició de \myref{def:familia-de-funcions-uniformement-equicontinua} trobem que la família de funcions~\(\{\varphi_{m+n_{0}}\}_{m\in\mathbb{N}}\) és uniformement equicontínua, i per l'observació \myref{obs:equicontinuitat-uniforme-implica-equicontinuitat} trobem que~\(\{\varphi_{m+n_{0}}\}_{m\in\mathbb{N}}\) és equicontínua.

        Aleshores pel \myref{thm:Teorema-dArzela-Ascoli} trobem que existeix una parcial uniformement convergent,~\((\varphi_{k})_{k\in\mathbb{N}}\), de~\((\varphi_{n_{0}+m})_{m\in\mathbb{N}}\), i per la definició de \myref{def:convergencia-uniforme} existeix una funció~\(\varphi\) tal que~\(\varphi_{k}\convergeixuniformement\varphi\), i per la proposició \myref{prop:una-successio-de-funcions-convergeix-uniformement-a-una-funcio-a-la-que-convergeixi-puntualment} trobem que la successió~\((\varphi_{k})_{k\in\mathbb{N}}\) convergeix puntualment a~\(\varphi\), i per la definició de \myref{def:convergencia-puntual} trobem que per a tot~\(t\in I\) tenim
        \begin{equation}
...
%                &=x_{0}+\int_{t_{0}}^{t}\lim_{k\to\infty}p_{k}(s,\varphi(s))\diff s, \\
        ...
        \end{align*}
        i per l'observació \myref{obs:solucio-general-dun-problema-de-Cauchy} tenim que~\(\varphi\) és una solució del problema de Cauchy \eqref{thm:Teorema-de-Peano:eq3}, i hem acabat.
    \end{proof}
    \begin{corollary}
    ...
    \begin{definition}[Conjunt de solucions d'un problema de Cauchy]
        \labelname{conjunt de solucions d'un problema de Cauchy}\label{def:conjunt-de-solucions-dun-problema-de-Cauchy}
        Sigui
        \begin{equation}
    ...
    \end{observation}
    \begin{proof}
        Per la definició de \myref{def:conjunt-de-solucions-dun-problema-de-Cauchy} tenim que~\(f\) és contínua i pel \myref{thm:Teorema-de-Peano} hem acabat.
    \end{proof}
    \begin{proposition}
    ...
    \begin{proof}
        ...
        \begin{enumerate}
            \item Reflexiva: Pel \myref{thm:doble-inclusio} tenim que~\(I\subseteq I\) i per la definició d'\myref{def:aplicacio} tenim que~\(u(x)=u(x)\) per a tot~\(x\) de~\(I\).
            Per tant trobem~\((I,u)\geq(I,u)\).

            \item Antisimètrica: Suposem que~\((I,u)\geq(J,v)\) i~\((J,v)\geq(I,u)\).
            Aleshores tenim que~\(J\subseteq I\) i~\(I\subseteq J\), i pel \myref{thm:doble-inclusio} trobem que~\(I=J\).
            També tenim que~\(u(x)=v(x)\) per a tot~\(x\) de~\(I\), i per la definició d'\myref{def:aplicacio} trobem que~\(u=v\), i per tant~\((I,u)=(J,v)\).
        ...
        \end{enumerate}
        I per la definició de \myref{def:relacio-dordre} hem acabat.
    \end{proof}
    \begin{definition}[Prolongació]
        \labelname{prolongació}\label{def:prolongacio}
        \labelname{solució improrrogable}\label{def:solucio-improrrogable}
        \labelname{solució maximal}\label{def:solucio-maximal}
    ...
    \begin{proof}
        ...
        Aleshores per la definició de \myref{def:prolongacio} tenim que per a tot~\(t\) de~\(I_{n}\) tenim que~\(u_{n}(t)=u_{m}(t)\), i per tant~\(u\) està ben definida.
        El cas~\((I_{m},u_{m})\leq(I_{n},u_{n})\) és anàleg.

        Ara bé, tenim que~\((I_{i},u_{i})\leq(I,u)\) per a tot~\(i\) de~\(J\), i
        per la definició de \myref{def:cota-superior-duna-cadena} tenim
        que~\((I_{i},u_{i})\leq(I,u)\) és un element maximal de la cadena~\((I_{i},u_{i})_{i\in J}\), i pel \myref{lema:lema-de-Zorn} tenim que existeix un element maximal de la cadena, i per la definició de \myref{def:solucio-improrrogable} hem acabat.
    \end{proof}
    \begin{proposition}
        \label{prop:les-solucions-improrrogables-estan-definides-sobre-intervals-oberts}
    ...
    \end{proposition}
    \begin{proof}
%            Sigui~\((I,u)\) una solució maximal del problema de Cauchy. Suposem que~\(a\) és un extrem de l'interval~\(I\). Considerem~\(A(a,u(a))\). Per la definició d'\myref{def:equacio-diferencial-ordinaria} trobem que~\((a,u(a))\in\Omega\), i per l'observació \myref{obs:el-conjunt-de-solucions-dun-problema-de-Cauchy-amb-f-continua-es-no-buit} trobem que existeix un~\((J,v)\in A(a,u(a))\).
%
%            Ara bé, tenim que~\(u(a)=v(a)\),
    \end{proof}
    \begin{proposition}
        \label{prop:la-solucio-improrrogable-es-unica}
        Sigui
        \[\begin{cases}
            \displaystyle \dot{u}(t)=f(t,u(t)) \\
            \displaystyle u(t_{0})=x_{0}
...
\section{Dependència de les solucions d'un problema de Cauchy}
    ...
    \begin{proof}
        ...
        Per la definició de \myref{def:limit} tenim que existeix un~\(\varepsilon>0\) tal que per a tot~\(N\) natural existeix un~\(n_{N}>N\) tal que
        \[
        ...
        \end{equation}
        per a tot~\(N\) natural.
        Ara bé, pel \myref{thm:Teorema-dArzela-Ascoli} la successió~\((\varphi_{n_{N}})_{N\in\mathbb{N}}\) té una parcial uniformement convergent, que contradiu \eqref{lema:continuitat-respecte-les-condicions-inicials:eq1}.
    ...
    \begin{definition}[Convergència uniforme sobre compactes]
        \labelname{convergència uniforme sobre compactes}\label{def:convergencia-uniforme-sobre-compactes}
    ...
    \end{proposition}
    \begin{proof}
        Observem primer que pel \myref{thm:Weierstrass-maxims-i-minims-multiples-variables} la funció~\(f(x)\) està acotada en~\(I\), i pel Teorema \myref{thm:Continua-acotada-implica-integrable-Riemann} tenim que la funció~\(f(x)\) és integrable Riemann, i per tant, pel \myref{thm:Teorema-Fonamental-del-Calcul}, trobem que per a tot~\(x_{0}\) de~\(I\)
        \[
            u(t)=\int_{x_{0}}^{x}f(u(t))\diff t
        \]
    ...
    \begin{proof}
        ...
        i per la proposició \myref{prop:Diferenciable-implica-continua} trobem que~\(\frac{1}{f(x)}\) és contínua.
        També tenim pel \myref{thm:Weierstrass-maxims-i-minims-multiples-variables} que~\(\frac{1}{f(x)}\) està acotada en~\((a,b)\) i pel Teorema \myref{thm:Continua-acotada-implica-integrable-Riemann} trobem que~\(\frac{1}{f(x)}\) és integrable en~\((a,b)\).
        Definim doncs %NO és en (a,b). És en (x_{0},x)
        \[
            F(x)=\int_{x_{0}}^{x}\frac{\diff t}{f(t)}.
        \]
        Pel \myref{thm:Teorema-Fonamental-del-Calcul} tenim que
        \begin{equation}
        ...
        \end{align*}
        I per tant, pel \myref{thm:Teorema-Fonamental-del-Calcul} trobem que
        \[
    ...
    \begin{proof}
        ...
        Ara bé, per la \myref{thm:regla-de-la-cadena} i el \myref{thm:Teorema-Fonamental-del-Calcul} tenim que això és
        \[
            H(x)=G(x)+C
        \]
        on~\(C\) és un real i
...
%            \[F(x)=\int_{x_{0}}^{x}\frac{\diff t}{f(t)}.\qedhere\]
    ...
    \begin{proof}
        ...
        \end{equation}
        Aleshores pel \myref{thm:Teorema-Fonamental-del-Calcul} tenim %I ref de la derivada del producte
        \[
