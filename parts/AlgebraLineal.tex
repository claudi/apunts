...
%		
%		Aleshores existeix una matriu \(P\) de mida \(m\times m\) sobre \(\mathbb{K}\) tal que \(f(A)=PA\) i existeix una matriu \(Q\) de mida \(n\times n\) sobre \(\mathbb{K}\) tal que \(g(A)=AQ\).
		\begin{proof}
	...
	\begin{definition}[Matriu esglaonada]
		...
		Sigui \(A=(a_{i,j})\) una matriu de mida \(m\times n\) sobre un cos \(\mathbb{K}\) tal que existeixen \(j_{1}<\dots<j_{r}\) nombres naturals tals que per a tot \(i\in\{1,\dots,r\}\) i per a tot \(j<j_{i}\) tenim \(a_{i,j}=0\) i \(a_{i,j_{i}}\neq0\), i \(a_{i,j}=0\) per a tot \(i>j_{r}\) i \(j\in\{1,\dots,n\}\).
