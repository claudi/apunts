...
%		\[\alpha\log\left(\frac{x}{x+1}\right)=\log\left(x^{\alpha}\right)-\alpha\log(x+1)\]
	...
	\begin{proof}
		...
		Per la definició de \myref{def:convergència absoluta d'una sèrie} tenim que la sèrie \(\sum_{n=0}^{\infty}a_{n}\) és absolutament convergent si i només si les sèries \(\sum_{n=0}^{\infty}a^{+}_{n}\) i \(\sum_{n=0}^{\infty}a^{-}_{n}\) són convergents. Per tant trobem que
		\begin{align*}
	...
	\end{theorem}
	\begin{proof}
		Prenem un \(\alpha\) real. Tenim per hipòtesi que la sèrie \(\sum_{n=0}^{\infty}a_{n}\) una sèrie convergent però no absolutament convergent, i pel lema \myref{lema:Teorema de la reordenació de sèries de Riemann} trobem que les sèries \(\sum_{n=0}^{\infty}a^{+}_{n}\) i \(\sum_{n=0}^{\infty}a^{-}_{n}\) són divergents.
...
\section{Sèries de funcions}
	...
	\label{subsec:sèries de potències}
	...
	\begin{definition}[Radi de convergència]
		...
		Siguin \(\sum_{n=0}^{\infty}a_{n}(x-x_{0})^{n}\) una sèrie de potències i \(R\) un real satisfent
		\begin{enumerate}
		...
		Aquesta definició té sentit pel Teorema~\myref{thm:radi de convergència d'una sèrie de potències}.
	\end{definition}
