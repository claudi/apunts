...
%			\[\alpha\log\left(\frac{x}{x+1}\right)=\log\left(x^{\alpha}\right)-\alpha\log(x+1)\]
	...
	\begin{example}
		...
		\begin{solution}
			...
			\begin{align*}
				\lim_{n\to\infty}n\left(1-\frac{\frac{\e^{n+1}(n+1)!}{(n+1)^{n+1}}}{\frac{\e^{n}n!}{n^{n}}}\right)&=\lim_{n\to\infty}n\left(\frac{\e^{n+1}(n+1)!n^{n}}{e^{n}n!(n+1)^{n+1}}\right) \\
				&=\lim_{n\to\infty}n\left(\frac{\e(n+1)!n^{n}}{(n+1)!(n+1)^{n}}\right) \\
				&=\lim_{n\to\infty}n\left(\frac{\e n^{n}}{(n+1)^{n}}\right) \\
				&=\e\lim_{n\to\infty}n\left(\frac{n}{n+1}\right)^{n} \\
				&=\e\lim_{n\to\infty}\left(\frac{n^{n+1}}{n+1}\right)^{n}=\infty
			\end{align*}
			i pel \myref{prop:criteri de Raabe} trobem que la sèrie \eqref{ex:quocient + Raabe:eq1} és convergent.
			
			Per tant la sèrie
			\[\sum_{n=1}^{\infty}\frac{\alpha^{n}n!}{n^{n}}\]
			és convergent si i només si \(\alpha\geq\e\).
		\end{solution}
	\end{example}
