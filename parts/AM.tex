...
%			Aquest enunciat té sentit pel Teorema \myref{thm:si una successió de funcions integrables Riemann convergix unifoemement, ho fa a una funció integrable Riemann}.
	...
	\begin{theorem}[Criteri de Dirichlet]
		...
		\[\sup_{x\in I}\abs{\sum_{n=0}^{\infty}}\leq M\quad\text{per a tot }n\text{ natural}.\]
...
%		Buscar la que va sortir a un dels exàmens. El de la recu?
	...
	\end{definition}
	\begin{definition}[Aproximació de l'unitat]
		\labelname{aproximació de l'unitat}\label{def:aproximació de l'unitat}
		...
		\begin{enumerate}
			...
			\item per a tot \(\delta>0\) tenim que la successió \((\phi_{\varepsilon})\) convergeix uniformement a \(0\) quan \(\varepsilon\) tendeix a \(0\) en l'interval \((-\delta,\delta)\).
		\end{enumerate}
		Aleshores direm que \((\phi_{\varepsilon})\) és una aproximació de l'unitat.
	\end{definition}
	\begin{lemma}
		\label{lema:Teorema d'aproximació polinòmica de Weierstrass}
		Sigui \(f\colon\mathbb{R}\longrightarrow\mathbb{R}\) una funció contínua amb suport compacte i \((\phi_{\varepsilon})_{\varepsilon\in\mathbb{R}}\) una aproximació de l'unitat. Aleshores \(f\convolucio\phi_{\varepsilon}\) convergeix uniformement a \(f\) quan \(\varepsilon\) tendeix a \(0\).
		\begin{proof}
...
%			Potser buscar un millor exemple pel criteri d'Abel
	...
	\begin{definition}[Gamma d'Euler]
		...
		\[\Gamma(x)=\int_{0}^{\infty}t^{n}\e^{-t}\diff t\]
	...
	\begin{theorem}
		...
		Es satisfà
		\[\lim_{x\to\infty}\frac{\Gamma(x+1)}{e^{-x}x^{x}\sqrt{2\pi x}}=1.\]
		\begin{proof}
			%TODO
		\end{proof}
	\end{theorem}
	...
	\begin{example}
		...
		\end{solution}
	\end{example}
	% Buscar un exemple sabroso a \url{http://www.math.uconn.edu/~kconrad/blurbs/analysis/diffunderint.pdf} per acabar tot aquest tema a lo grande.
...
%	\subsection{Els nombres complexos}
	...
	\begin{example}
		...
		\[f(t)=\sin(2\pi\omega t),\quad g(t)=\cos(2\pi\omega t)\quad\text{i}\quad h(t)=\e^{2\pi\iu\omega t}\]
	...
	\begin{example}
		...
		\[\{e_{n}(x)=e^{2\pi\iu nx}\mid n\in\mathbb{Z}\}\]
	...
	\begin{definition}[Coeficients de Fourier]
		...
		\[\widehat{f}(n)=\langle f(x),\e^{2\pi\iu nx}\rangle=\int_{0}^{1}f(x)\e^{-2\pi\iu nx}\diff x\]
	...
	\begin{proposition}
		...
		\[\widehat{f_{\tau}}(n)=\e^{-2\pi\iu n\tau}\widehat{f}(n).\]
		\begin{proof}
			%TODO
		\end{proof}
	\end{proposition}
	\begin{proposition}
		...
		\[\widehat{f'}(n)=2\pi\iu n\widehat{f}(n).\]
		\begin{proof}
			%TODO
		\end{proof}
	\end{proposition}
	...
	\begin{definition}[Sèrie de Fourier]
		...
		\[\sfourier(f)(x)=\sum_{n\in\mathbb{Z}}\widehat{f}(n)\e^{2\pi\iu nx}\]
