...
%			\[\alpha\log\left(\frac{x}{x+1}\right)=\log\left(x^{\alpha}\right)-\alpha\log(x+1)\]
	...
	\begin{proposition}[Criteri logarítmic]
		...
		\end{enumerate}
		\begin{proof}
			Comencem veient el punt \eqref{prop:criteri logarítmic:enum1}. Suposem doncs que \(L>1\). Prenem un \(\varepsilon>0\) tal que \(L-\varepsilon>1\). Aleshores per la definició de \myref{def:límit} tenim que existeix un natural \(n_{0}\) tal que per a tot \(n>n_{0}\) es satisfà
			...
			Veiem ara el punt \eqref{prop:criteri logarítmic:enum1}. Suposem doncs que \(L<1\). Prenem un \(\varepsilon>0\) tal que \(L+\varepsilon<1\).
		\end{proof}
	\end{proposition}
	\begin{example}
