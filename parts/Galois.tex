...
\subsection{Subanells engendrats per un element}
	...
	\begin{lemma}
		...
		\begin{proof} %FER %Revisar
			...
			\[\KK(\alpha)=\KK[\alpha].\]
			Per tant~\(\KK[\alpha]\) és un cos, i com que, per l'observació~\myref{obs:l'anell engendrat per alpha està entre l'extensió}, tenim que~\(\alpha\in\KK[\alpha]\), i per la definició de~\myref{def:cos per anells} tenim que~\(-1/\alpha\in\KK[\alpha]\). Aleshores, per la definició de~\myref{def:mínim subanell engendrat per un element d'una extensió} trobem que existeix un polinomi~\(a_{0}+a_{1}x+\dots+a_{k}x^{k}\in\KK[x]\), amb~\(a_{k}\neq0\), tal que
...
\subsection{Extensions algebraiques}
	...
	\begin{lemma}
		...
		\begin{proof}
			...
			Si~\(\mcd(p(x),q(x))\neq p(x)\), com que per la proposició~\myref{prop:en DIP irreductible implica primer} tenim que~\(p(x)\) és primer tenim que~\(\mcd(p(x),q(x))=1\), i per~\myref{thm:identitat de Bézout} trobem que existeixen~\(r(x)\) i~\(s(x)\) polinomis en~\(\KK[x]\) tals que
			\[p(x)r(x)+q(x)s(x)=1.\]
			Ara bé, tindríem que
			\[
...
\subsection{El grup de Galois}
	\begin{definition}[El grup de Galois]
		\labelname{grup de Galois}\label{def:grup de Galois}
