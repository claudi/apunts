...
\begin{document}
	...
	\begin{definition}[Mínim subgrup generat per un conjunt] %vspace
		...
		Siguin \(G\) un grup, \(S\) un subconjunt de \(G\), \(\{H_{i}\}_{i\in I}\) una família de subgrups de \(G\) tals que \(S\subseteq H_{i}\) per a tot \(i\in I\) i tal que
...
%			\marginpar{Si ningú ve del futur per aturar-te, com de dolenta pot ser la decisió que estàs prenent?}
	...
	\end{theorem}
	\begin{corollary}
		Si \(G\) té ordre \(p^{n}q^{m}\) on \(p,q\) són primers amb \(p<q\) aleshores \(n_{q}=1\), i pel corol·lari \myref{corollary:Segon Teorema de Sylow}, el \(p\)-subgrup de Sylow de \(G\) és un subgrup normal.
	\end{corollary}
