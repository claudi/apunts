...
%	\end{proposition}
	...
	% Veure que si \(f\) és decreixent (o creixent també?) en \(u(x)\) i contínua el problema de Cauchy té solució única. Xat amb l'Aure
	...
	\begin{proposition}
		...
		\begin{proof}
			...
			\begin{enumerate}
				\item Reflexiva: Pel \myref{thm:doble inclusió} tenim que \(I\subseteq I\) i per la definició d'\myref{def:aplicació} tenim que \(u(x)=u(x)\) per a tot \(x\) de \(I\). Per tant trobem \((u,I)\geq(u,I)\).
				
				\item Antisimètrica: Suposem que \((u,I)\geq(v,J)\) i \((v,J)\geq(u,I)\). Aleshores tenim que \(J\subseteq I\) i \(I\subseteq J\), i pel \myref{thm:doble inclusió} trobem que \(I=J\). També tenim que \(u(x)=v(x)\) per a tot \(x\) de \(I\), i per la definició d'\myref{def:aplicació} trobem que \(u=v\), i per tant \((u,I)=(v,J)\).
				
				\item Transitiva: Siguin \((u_{1},I_{1})\), \((u_{2},I_{2})\) i \((u_{3},I_{3})\) tres elements de \(A(t_{0},x_{0})\) tals que \((u_{1},I_{1})\geq(u_{2},I_{2})\) i \((u_{2},I_{2})\geq(u_{3},I_{3})\). Aleshores tenim que \(I_{3}\subseteq I_{2}\) i \(I_{2}\subseteq I_{1}\), i per la definició de \myref{def:subconjunt} trobem que \(I_{3}\subseteq I_{1}\).
	...
	\begin{definition}[Prolongació]
		...
		\labelname{solució improrrogable}\label{def:improrrogable}
