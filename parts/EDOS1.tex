...
%	\end{proposition}
	...
	\begin{theorem}[Teorema de Picard]
		...
		\begin{proof}
			...
			Per la definició de \myref{def:bola} trobem que \(T(u(x))(t)\) és un element de \(\overline{\bola}(x_{0},b)\), i per tant \(T(u(x))\) és un element de \(X\) per a tot \(u(x)\) de \(X\).
	...
	\end{definition}
	\begin{theorem}[Teorema de Arzelà-Ascoli]
		\labelname{Teorema de Arzelà-Ascoli}\label{thm:Teorema de Arzelà-Ascoli}
		...
		\begin{proof}
			Sigui \(\{f_{i}\}_{i\in I}\) una  família de funcions de \(\mathcal{C}_{b}(X)\). Prenem un real \(\varepsilon>0\). Per la definició de \myref{def:família de funcions equicontínua} tenim que per a tot punt \(x\) de \(X\) existeix un entorn \(N_{x}\) de \(x\) tal que per a tot \(y\) de \(N_{x}\) tenim, per a tot \(i\) de \(I\), que
			\[\norm{f_{i}(y)-f_{i}(x)}<\varepsilon.\]
			Ara bé, per la definició d'\myref{def:entorn} tenim que existeix un obert \(\obert{V}_{x}\) de \(X\) tal que \(x\) és un element de \(\obert{V}_{x}\) i \(\obert{V}_{x}\) és un subconjunt de \(N_{x}\). Per tant tenim que per a tot \(y\) de \(\obert{V}_{x}\), per a tot \(i\) de \(I\), es satisfà
			\[\norm{f_{i}(y)-f_{i}(x)}<\varepsilon.\]
