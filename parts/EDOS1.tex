...
%		\end{proof}
	...
	\begin{theorem}[Teorema Peano]
		...
		\begin{proof} % https://arxiv.org/pdf/1202.1152.pdf (no)
			...
			Fixem un \(\varepsilon>0\) i prenem dos \(t\) i \(t'\) de \(I\) tals que \(\abs{t,t'}<\frac{\varepsilon}{M}\). Aleshores per a tot \(m>n_{0}\) tenim que
			\[\norm{\varphi_{m}(t)-\varphi_{m}(t')}=\norm{\int_{t'}^{t}}\]
			
			\myref{def:família de funcions uniformement equicontínua}.
		\end{proof}
	\end{theorem}
	\begin{corollary}
...
%		\begin{proof}
%			Sigui \((I,u)\) una solució maximal del problema de Cauchy. Suposem que \(a\) és un extrem de l'interval \(I\). Aleshores podem considerar el mateix problema de Cauchy amb la condició inicial 
%		\end{proof}
%	\end{proposition}
	\begin{proposition}
		...
		\[\begin{cases}
			...
			\displaystyle u(t_{0})=x_{0}
		\end{cases}\]
		un problema de Cauchy sobre \(\Omega\) tal aquest tingui una única solució per a tot \((t_{0},x_{0})\in\Omega\). Aleshores per a cada \((t_{0},x_{0})\in\Omega\) aquest problema de Cauchy té una única solució improrrogable.
		\begin{proof}
	...
	\begin{lemma}
		...
		\begin{proof}
			Suposem que la successió \((\varphi_{n})_{n\in\mathbb{N}}\) no convergeix uniformement a \(\varphi\). Per la definició d'\myref{def:límit} tenim que existeix un \(\varepsilon>0\) tal que per a tot \(N\) natural existeix un \(n_{N}>N\) tal que
