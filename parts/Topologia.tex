...
\chapter{Les topologies}
	...
	\begin{definition}[Obert]
		...
		Sigui \(X\) amb la distància \(d\) un espai mètric i \(A\) un subconjunt de \(X\) tals que per a tot element \(a\) de \(A\) existeix un \(\varepsilon>0\) real tal que \(\B(a,\varepsilon)\subset A\). Aleshores direm que \(A\) és un obert.
	\end{definition}
	\begin{proposition}
	...
	\begin{example}[Topologia induïda per una mètrica]
		\labelname{}\label{ex:topologia induïda per una mètrica}
		...
		\begin{solution}
			...
			\[\bigcap_{i=1}^{n}\obert{U}_{i}\] %CLAUDI
		\end{solution}
	\end{example}
...
%	\end{proposition}
	...
	\begin{example}
		...
		\[\base{B}=\{\B(x,\varepsilon)\mid x\in X,\varepsilon>0\}\]
		un conjunt. Aleshores \(\base{B}\) és una base de la topologia \(\tau\).
	\end{example}
