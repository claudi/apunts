...
\section{Curvatura i torsió}
	...
	\begin{proposition}
		...
		\begin{proof}
			Per la definició de \myref{def:corba parametritzada per l'arc} trobem que
			\[\norm{\alpha'(t)}=1.\]
			Aleshores tenim que
			\begin{align*}
			...
			i per la definició de \myref{def:vectors perpendiculars} tenim que \(\alpha'\) i \(\alpha''\) són perpendiculars, com volíem veure.
		\end{proof}
%		\begin{proof}
...
%		\end{proof}
	...
	\begin{proposition}
		...
		\[\beta(t)=Q+\radicircosculadora_{\alpha}(t_{0})\left(-\normal_{\alpha}(t_{0})\cos\left(\frac{t}{\radicircosculadora_{\alpha}(t_{0})}\right)+\tangent_{\alpha}(t_{0})\sin\left(\frac{t}{\radicircosculadora_{\alpha}(t_{0})}\right)\right).\]
		\begin{proof}
			%TODO
...
%	\end{proposition}
	\begin{definition}[Primera forma fonamental]
		...
		\begin{align*}
			\PrimeraFormaFonamental_{p}\colon\EspaiTangent_{p}(S)\times\EspaiTangent_{p}(S)\colon&\longrightarrow\mathbb{R} \\
	...
	\begin{note}
		...
		\[\PrimeraFormaFonamental_{p}(\vec{v},\vec{w})=\vec{v}^{\transposta}\begin{bmatrix}
			E & F \\ F & G
		\end{bmatrix}\vec{w}\]
