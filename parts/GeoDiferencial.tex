...
\section{Superfícies regulars}
	...
	\begin{definition}[Carta local]
		...
		Sigui \(S\subseteq\mathbb{R}^{3}\) un conjunt i \(p\in S\)un punt tals que existeix un entorn obert \(\obert{U}\) de \(p\) i una aplicació \(\varphi\colon\Omega\longrightarrow S\cap\obert{U}\) que és una immersió i un homeomorfisme. Aleshores direm que \(\Omega\) amb \(\varphi\) és una carta local de \(p\) en \(S\).
	\end{definition}
	\begin{theorem}
	...
	\end{theorem}
	\begin{example}
		Volem veure que si tenim \(S\subseteq\mathbb{R}^{3}\) una superfície i \(\varphi\colon\Omega\longrightarrow\varphi(\Omega)\subseteq S\) una immersió injectiva, aleshores \(\Omega\) amb \(\varphi\) és una carta local si i només si \(\varphi\colon\Omega\longrightarrow\varphi(\Omega)\) és un homeomorfisme.
