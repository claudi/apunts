...
\section{Superfícies regulars}
	...
	\begin{definition}[Superfície]
		...
		Sigui \(S\subseteq\mathbb{R}^{3}\) un subconjunt tal que per a tot \(p\in S\) existeix un entorn obert \(\obert{U}\) de \(p\) i un difeomorfisme \(g\colon\obert{U}\longrightarrow g(\obert{U})\) tal que
	...
	\begin{theorem}
		...
		Sigui \(S\subseteq\mathbb{R}^{3}\) un subconjunt. Aleshores \(S\) és una superfície si i només si per a tot punt \(p\in S\) existeixen un entorn obert \(\obert{U}\) de \(p\) i una submersió \(F\colon\obert{U}\longrightarrow\mathbb{R}\) tals que
		\[S\cap\obert{U}=F^{-1}(\{0\}).\]
		\begin{proof}
			Comencem veient que la condició és suficient (\(\implica\)). Suposem doncs que \(S\) és una superfície. Per la definició de \myref{def:superfície} trobem que que per a tot \(p\in S\) existeix un entorn obert \(\obert{U}\) de \(p\) i un difeomorfisme \(g\colon\obert{U}\longrightarrow g(\obert{U})\) tal que
			\begin{equation}
...
%			continuar
	...
	\end{theorem}
	\begin{example}
		Exemple d'alguna superfície amb el teorema aquest.
		\begin{solution}
			%TODO
		\end{solution}
	\end{example}
...
%			\[\pi_{1}\circ g\circ\Psi_{1}\]
	...
	\end{definition}
%	\begin{proposition}
%		\label{prop:les derivades d'una parametrització en una carta local són base del pla}
%		Siguin \(S\subseteq\mathbb{R}^{3}\) una superfície, \(p\in S\) un punt i \(\Psi(u,v)\) amb \(\Omega\) una carta local de \(S\) en \(p\). Aleshores
%		\[\left(\frac{\partial \Psi}{\partial u},\frac{\partial \Psi}{\partial v}\right)\]
%		és una base de \(\mathbb{R}^{2}\).
%	\end{proposition}
	\begin{definition}[Primera forma fonamental]
		...
		\begin{align*}
			\I_{p}\colon\EspaiTangent_{p}(S)\times\EspaiTangent_{p}(S)\colon&\longrightarrow\mathbb{R} \\
			(\vec{u},\vec{w})&\longmapsto\prodesc{\vec{v}}{\vec{w}}
		\end{align*}
		és la primera forma fonamental de \(S\) en \(p\).
	\end{definition}
	\begin{note}
		Després justificaré això però tenim
		\[\I_{p}(\vec{v},\vec{w})=\vec{v}^{\transposta}\begin{bmatrix}
			E & F \\ F & G
		\end{bmatrix}\vec{w}\]
		i si \(\Psi=\Psi(u,v)\), denotant
		\[\Psi_{u}=\frac{\partial\Psi}{\partial u}\quad\text{i}\quad\Psi_{v}=\frac{\partial\Psi}{\partial v}\]
		tenim
		\[\begin{bmatrix}
			E & F \\ F & G
		\end{bmatrix}=
		\begin{bmatrix}
			\prodesc{\Psi_{u}}{\Psi_{u}} & \prodesc{\Psi_{u}}{\Psi_{v}} \\ \prodesc{\Psi_{u}}{\Psi_{v}} & \prodesc{\Psi_{v}}{\Psi_{v}}
		\end{bmatrix}.\]
	\end{note}
