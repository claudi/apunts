...
%		\ref{def:diferencial}
	...
	\begin{definition}[Curvatura]
		...
		Si \(\curvatura_{\alpha}(t)\neq0\) direm que \(\alpha\) té curvatura no nu{\lgem}a en \(t\).
	\end{definition}
	\begin{example}
	...
	\begin{proposition}
		...
		Sigui \(\alpha\) una corba sobre \(I\) amb curvatura no nu{\lgem}a i \(t\in I\) un real. Aleshores els vectors \(\tangent_{\alpha}(t)\) i \(\normal_{\alpha}(t)\) són perpendiculars.
		\begin{proof}
			Considerem
...
%			\[\beta'(t)=\]
	...
	\begin{proposition}
		...
		\begin{proof}
			...
			Per l'observació \myref{obs:la normal a una corba és unitària} i la definició de \myref{def:corba parametritzada per l'arc} tenim que \(\norm{\tangent_{\alpha}(t)}=\norm{\normal_{\alpha}(t)}=1\), i em queda veure que \(\norm{\binormal_{\alpha}(t)}=1\). % FER
		\end{proof}
	\end{proposition}
	\begin{definition}[Triedre de Frenet]
		\labelname{triedre de Frenet}\label{def:triedre de Frenet}
		Sigui\(\alpha\) una corba amb curvatura no nu{\lgem}a. Aleshores direm que la base \((\tangent_{\alpha}(t),\normal_{\alpha}(t),\binormal_{\alpha}(t))\) és el triedre de Frenet.
		
		Aquesta definició té sentit per la proposició \myref{prop:triedre de Frenet}.
	\end{definition}
	\begin{proposition}
		\label{prop:torsió}
		Sigui \(\alpha\) una corba amb curvatura no nu{\lgem}a. Aleshores existeix un \(\torsio_{\alpha}(t)\in\mathbb{R}\) tal que
		\[\binormal'_{\alpha}(t)=\torsio_{\alpha}(t)\normal(t).\]
		\begin{proof}
			Tenim per la proposició \myref{prop:triedre de Frenet} que \((\tangent_{\alpha}(t),\normal_{\alpha}(t),\binormal_{\alpha}(t))\) és una base ortonormal i positiva de \(\mathbb{R}^{3}\), i per tant tenim que %REF
			...
			Ara bé, per la definició de \myref{def:binormal} trobem que
			\[\binormal'_{\alpha}(t)=\frac{\diff(\tangent_{\alpha}(t)\prodvec\normal_{\alpha}(t))}{\diff t},\]
			i per la proposició \myref{prop:fórmula de Leibniz} tenim que
			\[\binormal'_{\alpha}(t)=\tangent'_{\alpha}(t)\prodvec\normal_{\alpha}(t)+\tangent_{\alpha}(t)\prodvec\normal'_{\alpha}(t).\]
			Aleshores per la proposició
			\[hmm\qedhere\]
		\end{proof}
	\end{proposition}
	\begin{definition}[Torsió]
		Sigui \(\alpha\) una corba amb curvatura no nu{\lgem}a i \(\torsio_{\alpha}(t)\) un real tal que
		\[\binormal'_{\alpha}(t)=\torsio_{\alpha}(t)\normal(t).\]
