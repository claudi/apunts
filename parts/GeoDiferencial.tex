...
\section{Superfícies}
	...
	\begin{proposition}
		...
		\begin{proof}
			...
			\[d(f\circ g)=dgdf(g),\] % REFS
			i per tant trobem que \(\rang(d(f\circ g))\leq\min\{\rang(dg),\rang(df(g))\}\), i per l'observació \myref{obs:immersió si i només si té rang maximal} hem acabat.
		\end{proof}
	\end{proposition}
	...
	\begin{theorem}[Teorema d'estructura local de les immersions]
		...
		\begin{proof}
			...
			\begin{equation}
				...
				\det(dF(x_{0}))\neq0.
			\end{equation}
			També tenim que \(g(x,0)=F(x)\), i per l'exemple \myref{ex:la inclusió canònica és una immersió} trobem que
			...
			\[\det(dg(x_{0})\neq0,\]
			i pel \myref{thm:Funció implícita} hem acabat.
		\end{proof}
	\end{theorem}
